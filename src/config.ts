...
export async function loadConfig(): Promise<CcstatConfig> {
	try {
		...
		const config = JSON.parse(content) as Partial<CcstatConfig & { USEOBSERVED?: boolean }>;

		// Validate required fields and provide defaults
		const validatedConfig = {
			TOKEN_LIMIT: config.TOKEN_LIMIT ?? DEFAULT_CONFIG.TOKEN_LIMIT,
			OBSERVED_MAX_TOKEN: config.OBSERVED_MAX_TOKEN ?? DEFAULT_CONFIG.OBSERVED_MAX_TOKEN,
			USE_OBSERVED_IF_HIGHER:
				config.USE_OBSERVED_IF_HIGHER ??
				config.USEOBSERVED ??
				DEFAULT_CONFIG.USE_OBSERVED_IF_HIGHER,
			BURN_RATE_HIGH_THRESHOLD:
				config.BURN_RATE_HIGH_THRESHOLD ?? DEFAULT_CONFIG.BURN_RATE_HIGH_THRESHOLD,
			BURN_RATE_MODERATE_THRESHOLD:
				config.BURN_RATE_MODERATE_THRESHOLD ?? DEFAULT_CONFIG.BURN_RATE_MODERATE_THRESHOLD,
			REFRESH_INTERVAL_MS: config.REFRESH_INTERVAL_MS ?? DEFAULT_CONFIG.REFRESH_INTERVAL_MS,
			...
			TIME_CRITICAL_THRESHOLD:
				config.TIME_CRITICAL_THRESHOLD ?? DEFAULT_CONFIG.TIME_CRITICAL_THRESHOLD,
			USAGE_WARNING_THRESHOLD:
				config.USAGE_WARNING_THRESHOLD ?? DEFAULT_CONFIG.USAGE_WARNING_THRESHOLD,
			USAGE_CRITICAL_THRESHOLD:
				config.USAGE_CRITICAL_THRESHOLD ?? DEFAULT_CONFIG.USAGE_CRITICAL_THRESHOLD,
			CONTEXT_WARNING_THRESHOLD:
				config.CONTEXT_WARNING_THRESHOLD ?? DEFAULT_CONFIG.CONTEXT_WARNING_THRESHOLD,
			CONTEXT_CRITICAL_THRESHOLD:
				config.CONTEXT_CRITICAL_THRESHOLD ?? DEFAULT_CONFIG.CONTEXT_CRITICAL_THRESHOLD,
			DEBUG_OUTPUT:
				(process.env.DEBUG_OUTPUT === "true" || config.DEBUG_OUTPUT) ?? DEFAULT_CONFIG.DEBUG_OUTPUT,
		};
		...
		if (shouldMigrate) {
			...
			if (validatedConfig.DEBUG_OUTPUT) {
				console.log("📦 Migrated config from ~/ccstat.json to ~/.config/ccstat.json");
			}
		}
...
export function getEffectiveTokenLimit(config: CcstatConfig): number {
	if (config.USE_OBSERVED_IF_HIGHER && config.OBSERVED_MAX_TOKEN > config.TOKEN_LIMIT) {
