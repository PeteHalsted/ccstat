 ...
 *
 * Original data loading and validation logic copied and adapted for ccstat
 */
...
const isoTimestampSchema = z.string().refine((val: string) => !Number.isNaN(Date.parse(val)), {
...
export async function loadUsageEntries(debugOutput: boolean = false): Promise<LoadUsageResult> {
	if (debugOutput) {
		console.log(`DEBUG: Starting loadUsageEntries with debugOutput=${debugOutput}`);
	}
		...
		for (const project of projects) {
			...
			if (fs.statSync(projectPath).isDirectory()) {
				...
				if (debugOutput && files.length > 0) {
					console.log(`DEBUG: Found ${files.length} .jsonl files in ${projectPath}`);
				}
				...
				for (const filePath of files) {
					...
					for (const line of lines) {
						try {
							...
							if (!contextSession) {
								throw new Error(`Failed to get context session for project: ${projectName}`);
							}
...
export async function aggregateSessionData(): Promise<SessionData> {
	...
	const totalCacheCreationTokens = entries.reduce((sum, e) => sum + e.cacheCreationTokens, 0);
	const totalCacheReadTokens = entries.reduce((sum, e) => sum + e.cacheReadTokens, 0);
	const totalCostUSD = entries.reduce((sum, e) => sum + e.costUSD, 0);

	// Get unique values
	const projectNames = [...new Set(entries.map((e) => e.projectName))];
	const models = [...new Set(entries.map((e) => e.model).filter((m) => m !== null))];
