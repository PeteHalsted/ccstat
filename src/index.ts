...
// ---- Configuration ----
const HIGH_WATER_MARK_FILE = path.join(USER_HOME_DIR, ".claude_status_max_tokens");
...
function calculateBurnRate(block: SessionBlock) {
	...
	const durationMinutes =
		(lastEntry.timestamp.getTime() - firstEntry.timestamp.getTime()) / (1000 * 60);
...
function projectBlockUsage(block: SessionBlock) {
...
async function main() {
	...
	if (config.DEBUG_OUTPUT) {
		console.log(`DEBUG: Config loaded with DEBUG_OUTPUT=${config.DEBUG_OUTPUT}`);
	}
	...
	while (true) {
		try {
			...
			const blocks = identifySessionBlocks(entries, config.DEFAULT_SESSION_DURATION_HOURS);

			if (config.DEBUG_OUTPUT) {
				console.log(`DEBUG: Total entries loaded: ${entries.length}`);
				console.log(`DEBUG: Total blocks created: ${blocks.length}`);
				console.log(`DEBUG: Block entry counts: ${blocks.map((b) => b.entries.length).join(", ")}`);
			}
			...
			for (let i = pathParts.length; i > 0; i--) {
				...
				const foundSession = contextSessions.find((s) => s.sessionId?.includes(normalizedProjName));
				if (foundSession) {
			...
			if (activeBlock) {
				...
				const usedPct = Math.round((blockTotalTokens * 100) / EFFECTIVE_TOKEN_LIMIT);
				usedDisplay = `Used: ${usedPct}%`;

				const projectedTokens = projection?.totalTokens || 0;
				const projectedPct = Math.round((projectedTokens * 100) / EFFECTIVE_TOKEN_LIMIT);
			...
			console.log(`  ${burnRateDisplay} | ${usedDisplay} | ${projectedDisplay}\x1B[K`);
			...
			if (config.DEBUG_OUTPUT && activeContextSession) {
				...
				console.log(`  cacheReadTokens: ${activeContextSession.cacheReadTokens}`);
			...
			if (activeBlock && config.DEBUG_OUTPUT) {
				...
				console.log(`  First Entry: ${activeBlock.entries[0]?.timestamp.toISOString() || "N/A"}`);
				console.log(
					`  Last Entry: ${activeBlock.entries[activeBlock.entries.length - 1]?.timestamp.toISOString() || "N/A"}`
				);
				console.log(`  Input: ${activeBlock.tokenCounts.inputTokens?.toLocaleString() || 0}`);
				console.log(`  Output: ${activeBlock.tokenCounts.outputTokens?.toLocaleString() || 0}`);
				console.log(
				...
				console.log(`  Usage %: ${Math.round((blockTokens * 100) / EFFECTIVE_TOKEN_LIMIT)}%`);
				if (projection) {
					console.log(`  Projected Total: ${projection.totalTokens.toLocaleString()}`);
					console.log(
